...
class Bank extends Bank_controller
{
    ...
    public function index(){
        $data = array();
        $this->template->title(translate('SMSes'))->build('bank/index',$data);
    }
    ...
    function listing(){
        ...
        $this->data['from'] = $from;
        $this->data['to'] = $to;
        $this->template->title('Sent SMSes')->build('bank/listing',$this->data);
    }
    ...
    function create(){
        ...
        if($this->form_validation->run()){
            ...
            if($message_id){
                if($this->input->post('new_item')){
                    redirect('bank/sms/create','refresh');
                }else{
                     redirect('bank/sms/queued_sms','refresh');
                }
            }
            else{
                redirect('bank/sms/listing','refresh');
            }
        ...
        $this->template->title('Create SMS')->build('bank/form',$this->data);
    }
    ...
    function queued_sms(){
        ...
        $this->data['from'] = $from;
        $this->data['to'] = $to;
        $this->template->title('Queued Group SMSes')->build('bank/queued_sms',$this->data);
    }

    function delete($id=0,$redirect=TRUE){
        $id OR redirect('bank/sms?a=queued_sms');
        $post = $this->sms_m->get_queued_sms($id);
        if(!$post){
            $this->session->set_flashdata('error','Sorry, the sms does not exist');
            if($redirect){
                redirect('bank/sms?a=queued_sms');
            }
            return FALSE;
        }
