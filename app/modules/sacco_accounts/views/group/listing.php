...
<script>
                            ...
                            $.ajax({
                                ...
                                success: function(response){
                                    if(isJson(response)){
                                        ...
                                        mApp.unblock('.'+id+'_active_row');
                                        swal("Cancelled", "Could not delete your sacco account :)", "error")   
                                    }
                                },
                                error: function(){
                                    mApp.unblock('.'+id+'_active_row');
                                    swal("Cancelled", "Could not delete your contributions :)", "error")
                                },
                            });
                        }
                    });
                }else{
                    swal("Cancelled", "Your sacco account is safe :)", "error")
                }
            })
        });
    ...
    function load_sacco_accounts_listing(){
        mApp.block('#sacco_accounts_listing',{
            overlayColor: 'white',
            animate: true
        });
        $.ajax({
            type: "GET",
            url: '<?php echo base_url("ajax/sacco_accounts/get_sacco_accounts_listing/'+first_uri_segment+'/'+second_uri_segment+'?'+get_string+'"); ?>',
            dataType : "html",
                success: function(response) {
                    $('#sacco_accounts_listing').html(response);
                    $('.select2').select2({width:"100%"});
                    $('.date-picker').datepicker({autoclose:true});
                    mApp.unblock('#sacco_accounts_listing');
                }
            }
        );
    }
