...
class Admin extends Admin_Controller
{
    ...
    {
        $id OR redirect('admin/menus/listing');

        $post = $this->menus_m->get($id);

        if(!$post)
        {
            $this->session->set_flashdata('error','Sorry, the Admin Menu does not exist');
            redirect('admin/menus/listing');
        }

        $id = $this->menus_m->delete($post->id);
    ...
    function hide($id=0,$redirect=TRUE)
    {
        ...
        $id = $this->menus_m->update($post->id,array
            (
                'active'=>NULL,
                'modified_on' => time(),
    ...
    function listing()
    {
        $this->data['posts'] = $this->menus_m->get_parent_links();
        $this->data['side_bar_menu_options'] = $this->menus_m->get_options();
    ...
    function create()
    {
        ...
        $this->data['menus'] = $this->menus_m->get_options();
        $this->data['post'] = $post;
    ...
    function edit($id=0)
    {
        ...
        if($this->form_validation->run())
        {
            ...
            $update = $this->menus_m->update($id,$data);
            if($update)
            {
                ...
                {
                    redirect('admin/menus/listing','refresh');
                }
            }
            else
            {
    ...
    function ajax_sort_update()
    {
        ...
        for($i=0;$i<count($data);$i++){
            $this->menus_m->update($data[$i]->id,array(
                'position'=>$i,
                'modified_on' => time(),
                'modified_by' => $this->ion_auth->get_user()->id,
            ));
    ...
    private function _children($pt,$parent_id,$position){
        ...
        $this->menus_m->update($pt->id,array(
            'position'=>$position,
            'parent_id'=>$parent_id,
            'modified_on' => time(),
            'modified_by' => $this->ion_auth->get_user()->id,
