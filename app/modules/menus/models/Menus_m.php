...
class Menus_m extends MY_Model {
	...
	public function __construct(){
		...
		$this->notification_counts = array(
			...
			'PENDING_WITHDRAWAL_APPROVAL_REQUESTS_COUNT_AND_RECONCILE' => isset($this->pending_withdrawal_approval_requests_count)?$this->pending_withdrawal_approval_requests_count_and_reconcile:'',
		);
	...
	**/
	...
	function grand_child_is_active($parent_id,$grand_child_link_url){
		...
		if($grand_child_link_url){
			...
			$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
			$parent_menu = $this->db->get('menus')->row();	
			if($parent_menu){
				$this->db->select(array($this->dx('parent_id').' as parent_id '));
	...
	function great_grand_child_is_active($parent_id,$great_grand_child_link_url){
		...
		if($great_grand_child_link_url)
		{
			...
			$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
			$grand_child_menu = $this->db->get('menus')->row();
			if($grand_child_menu)
			{
				$this->db->select(array($this->dx('parent_id').' as parent_id '));
	...
	function generate_side_bar_menu(){
		...
		$this->notification_counts = array(
			...
			'PENDING_WITHDRAWAL_APPROVAL_REQUESTS_COUNT_AND_RECONCILE' => $this->pending_withdrawal_approval_requests_count_and_reconcile,
		);
		if($this->member_role_permissions){
			$menus_array=array();
			foreach ($this->member_role_permissions as $permission) {

				$menus_array[$permission->menu_id] = $permission->menu_id;
				...
				if(in_array($parent_menu_id,$menus_array)){

				}else{
					$menus_array[$parent_menu_id] = $parent_menu_id;
				}
			}
			$string = 'group';
			$dashboard_id = $this->get_dashboard_id();
			$menus_array=array($dashboard_id=>$dashboard_id)+$menus_array;
			if($menus_array){
				$parent_new = array();
				foreach ($menus_array as $menu_id) {
					...
					if($link && $link->parent_id==0){
						...
						if($parent->parent_id!=0){
							...
							$menus_array = $menus_array + array($add_menu->id=>$add_menu->id);
							if($parent->parent_id==0){
								$parent=$add_menu;
							}else{
				...
				if($value){
					...
					foreach ($value as $parent) {
						...
						$show = 1;
						$specific = 0;
						if($parent->enable_menu_for){
							$specific = 1;
							$feature = $parent->enabled_disabled_feature;
							if(isset($this->group->$feature) && $this->group->$feature){
								if($parent->enabled_or_disabled == 1){//show menu
									$show = 1;
								}else if($parent->enabled_or_disabled == 2){//do not show menu
									$show = 0;
								}
							}else{
								if($parent->enabled_or_disabled == 1){//show menu
									...
									echo '">
										...
										foreach ($this->notification_counts as $k => $v){
											...
											if($k == 'UNRECONCILED_DEPOSITS_COUNT'){
												...
												$count_class = " withdrawals_count ";
											}else if($k == 'PENDING_WITHDRAWAL_APPROVAL_REQUESTS_COUNT_AND_RECONCILE'){
												$count_class = " withdrawal_approval_count ";
											}
										...
										echo '</a>';
										$this->generate_side_bar_sub_menu($parent->id,$menus_array,TRUE);
									echo'</li>';
								}
							}
						}else{
						...
						if($specific){
							...
							if($this->billing_settings->menu_acceptable_for_package($this->group->billing_package_id,$parent->id)){
								echo '<li class="m-menu__section li_searchable_hidden">';
									...
									echo '<i class="m-menu__section-icon '.$parent->icon.'"></i>';
									echo'</li>';
									$this->generate_sub_menu($parent->id,$menus_array,TRUE);
							}
						}
					}
					echo '
				...
				echo '
					<ul class="m-menu__nav  m-menu__nav--dropdown-submenu-arrow ">
				        <div class="toggle_menu_search">
			       		...
			       		<li class="m-menu__item '.$active.'" aria-haspopup="true"  m-menu-link-redirect="1"><a  href="'.site_url('group').'" class="m-menu__link "><i class="m-menu__link-icon mdi mdi-view-dashboard"></i><span class="m-menu__link-text">'.translate('Dashboard').'</span></a></li>
				';
				foreach($parent_links as $link){
					$href = $link->url?site_url($link->url):'javascript:;';
					if(preg_match('/javascript/', $href)){
	...
	function generate_page_title_menus($url=''){
		...
		if($menu){
			...
			if(in_array('listing', $strings)){
				$num = 1;
			}elseif(in_array('connect', $strings)){
				$num = 2;
			}
			if($count>2){
				$max = $count-$num;
	...
	function generate_children_title_menus($id=0,$url='',$set_active=''){
		$this->notification_counts = array(
			...
			'PENDING_WITHDRAWAL_APPROVAL_REQUESTS_COUNT' => $this->pending_withdrawal_approval_requests_count,
			'PENDING_WITHDRAWAL_APPROVAL_REQUESTS_COUNT_AND_RECONCILE' => $this->pending_withdrawal_approval_requests_count_and_reconcile,
			'GROUP_MEMBERSHIP_REQUESTS_COUNT' => $this->group_membership_requests_count,
		);
