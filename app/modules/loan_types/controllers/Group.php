...
class Group extends Group_Controller{
    ...
    // function index(){
    //     $this->template->title('Group Loan Types')->build('group/index',$this->data);
    // }
    ...
    function create(){
        ...
        $this->data['post'] = $post;
    	$this->template->title('Create Loan Type')->build('group/form',$this->data);
    }

    function index(){
    	$posts = $this->loan_types_m->get_all();
        $admin_loan_types = $this->loan_types_m->get_admin_loan_types_for_group($this->group->id);
        $posts = $admin_loan_types?array_merge($posts,$admin_loan_types):$posts;
    	$this->data['posts'] = $posts;
    	$this->template->title(translate('Loan Types'))->build('group/listing',$this->data);
    }

    function listing(){
    	redirect('group/loan_types');
    }
