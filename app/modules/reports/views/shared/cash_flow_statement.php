...
<script type="text/javascript">
    ...
    function load_cash_flow_statement(){
        mApp.block('#cash_flow_statement',{
            overlayColor: 'white',
            animate: true
        });
        $.ajax({
            type: "GET",
            url: '<?php echo base_url("ajax/reports/get_cash_flow_statement/'+first_uri_segment+'/'+second_uri_segment+'?'+get_string+'"); ?>',
            dataType : "html",
                success: function(response) {
                    $('#cash_flow_statement').html(response);
                    $('#statement_footer').slideDown();
                    mApp.unblock('#cash_flow_statement');
                }
            }
        );
    }
