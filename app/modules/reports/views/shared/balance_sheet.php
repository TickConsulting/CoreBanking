<div class="invoice-content-2 bordered document-border">
                            ...
                            <div class="m-dropdown__content">
                                <?php echo form_open(current_url(),'method="GET" class="filter m-form m-form--label-align-right"');?>
                                    <div class="form-group m-form__group row">
                                        <div class="col-lg-12">
                                            <label>
                                                <?php echo translate('Balance Sheet Date Range'); ?>
                                            </label>
    ...
    <div class="clearfix"></div>
    <hr/>
    <div class="col-xs-12"  id="print_holder">
        ...
        &nbsp;&nbsp;&nbsp;
        <?php 
            $query = $_SERVER['QUERY_STRING']?'&generate_pdf=1':'?generate_pdf=1';
            $search_string = substr(basename($_SERVER['REQUEST_URI']).$query,strpos(basename($_SERVER['REQUEST_URI'].$query), "?"));?>
        <button class="btn btn-sm btn-primary uppercase" id="generate_pdf" href="javascript:;" ><i class='fa fa-file'></i>
...
<script type="text/javascript">
    ...
    function load_balance_sheet(){
        mApp.block('#balance_sheet',{
            overlayColor: 'white',
            animate: true
        });
        $.ajax({
            type: "GET",
            url: '<?php echo base_url("ajax/reports/get_balance_sheet/'+first_uri_segment+'/'+second_uri_segment+'?'+get_string+'"); ?>',
            dataType : "html",
