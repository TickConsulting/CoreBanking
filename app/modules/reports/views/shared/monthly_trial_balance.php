<div class="invoice-content-2 bordered document-border">
    ...
    <div id="statement_paper"  class="pt-3">
        <div id="monthly_trial_balance" class="pt-3">
        </div>
        <hr> 
...
<script type="text/javascript">
    ...
    function load_monthly_trial_balance(){
        mApp.block('#monthly_trial_balance',{
            overlayColor: 'white',
            animate: true
        });
        $.ajax({
            type: "GET",
            url: '<?php echo base_url("ajax/reports/get_monthly_trial_balance/'+first_uri_segment+'/'+second_uri_segment+'?'+get_string+'"); ?>',
            dataType : "html",
                success: function(response) {
                    $('#monthly_trial_balance').html(response);
                    $("#statement_footer").slideDown();
                    mApp.unblock('#monthly_trial_balance');
