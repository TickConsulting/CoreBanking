...
class Admin extends Admin_Controller
{
	...
	protected $validation_rules = array(
			...
			array(
					'field'	=>	'slug',
					'label'	=>	'Group Slug',
					'rules'	=>	'trim|required|callback__is_unique_slug',
				),
			array(
					'field'	=>	'size',
					'label'	=>	'Group size',
			...
			array(
					'field'	=>	'enable_edit_member_profile',
					'label'	=>	'Enable Edit Member Profile',
					'rules'	=>	'required|numeric',
				),
			array(
					'field'	=>	'enable_compose_sms',
					'label'	=>	'Enable Compose SMS',
					'rules'	=>	'required|numeric',
				),
			array(
					'field'	=>	'enable_compose_email',
					'label'	=>	'Enable Compose Email',
					'rules'	=>	'required|numeric',
				),
		);
	...
	public function __construct(){
		...
		$this->load->model('migrate/migrate_m');
	...
	function search($id = 0)
	{
		...
		$this->template->title('Group Search')->build('admin/search',$this->data);
	}
	...
	function listing(){	
		...
		if($bank_ids){
			$group_ids=$this->groups_m->get_group_ids_by_bank_ids($bank_ids);
			
			foreach ($group_ids as $group_id) {
				array_push($group_ids_arr,$group_id['group_id']);
			}
		}
   ...
   //      print_r($total_rows);
		...
		$this->data['currency_options'] = $this->countries_m->get_currency_code_options();
		$this->data['setup_tasks'] = array(
			'' => 'Unknown / Imported',
			'0' => 'Setup Error',
			'1' => 'Create Group',
			'2' => 'Membership',
			...
			'6' => 'Final Step - Confirmation',
		);
		$this->data["admin_users"] = $this->users_m->get_user_group_options(3);

		if($this->input->get('generate_excel') == 1){
			$this->data['bank_accounts'] = $this->bank_accounts_m->get_groups_bank_accounts($posts);
			foreach ($posts as $post) {
				$post->phone = '';
				$post->email = '';
				$post->username = ' --Not Set--';
				if($post->owner){
			...
			$response = $this->excel_library->generate_group_listing_report($json_file);
		...
		$this->data['bank_accounts'] = $this->bank_accounts_m->get_groups_bank_accounts($posts);
		$this->template->title('Group List')->build('admin/listing',$this->data);
	}
	...
	function edit($id=0){
		...
		if($this->form_validation->run()){
			$update = $this->groups_m->update($post->id,
					array(
						...
						'enable_edit_member_profile' => $this->input->post('enable_edit_member_profile'),
						'enable_compose_sms' => $this->input->post('enable_compose_sms'),
						'enable_compose_email' => $this->input->post('enable_compose_email'),
						'enable_add_members_manually' => $this->input->post('enable_add_members_manually'),
						'enable_import_members_manually' => $this->input->post('enable_import_members_manually')
					)
				);
			if($update)
			{
			...
			$this->billing_settings->update_group_subscription_status($post->id);
        ...
        $this->data['billing_packages'] = array();//$this->billing_m->billing_packages_options();
		$this->data['id'] = $id;
	...
	function view($id=0){
		...
		$this->template->title($post->name)->build('admin/view',$this->data);
	}
	...
	function ajax_view($id=0){
		...
		if($post){
			...
			//print_r($regular_contribution_settings_array);die;
	        ...
	        $subscription_status = isset($this->billing_settings->subscription_statuses[$post->subscription_status])?($this->billing_settings->subscription_statuses[$post->subscription_status]?:'--Not Set--'):'--Not Set--';
	        ...
	        <div>
				...
				if($post->disable_notifications){
					echo '<a href="'.site_url('admin/groups/enable_notifications/'.$post->id).'" class="btn btn-xs btn-info " >
	                        <i class="icon-volume-2"></i> Enable Notifications
	                    </a>&nbsp;&nbsp;';
				}else{
					echo '<a href="'.site_url('admin/groups/disable_notifications/'.$post->id).'" class="btn btn-xs btn-info " >
	                        <i class="icon-volume-off"></i> Disable Notifications
	                    </a>&nbsp;&nbsp;';
				}
	            echo '<a href="'.site_url('admin/groups/export_group_data/'.$post->id).'" class="btn btn-xs btn-primary " >
	        	...
	        	echo '
	            <a target="_blank" href="'.site_url('admin/groups/login_as_admin/'.$post->id).'" class="btn btn-xs btn-default">
	                                        ...
	                                        <li class="task-list-item done">
	                                            ...
	                                            <div class="task-content">
	                                                ...
	                                                <ul class="">
	                                                    ...
	                                                    <li><strong>Group Subscription Status: </strong>'.$subscription_status.'</li>
	                                                    <li><strong>Group Last Activity: </strong>'.timestamp_to_message_time($post->last_group_activity).'</li>
	                                                    <li><strong>Next Billing Date: </strong>'.timestamp_to_date($post->billing_date).'</li>
	                                                </ul>
	                                            </div>
	                                        </li>
	                                                ...
	                                                <p>';
	                                                    if($bank_accounts){
	                                                        ...
	                                                        foreach ($bank_accounts as $bank_account):
	                                                            echo '<li>'; echo'<strong>'.$bank_account->bank_name.'</strong> | '.$bank_account->bank_branch.' branch | '.$bank_account->account_name.' | '.$bank_account->account_number; 
	                                                            	if($bank_account->partner){
	                                                            		if($bank_account->is_verified){
	                                                            			echo '
															            	<a href="'.site_url('admin/bank_accounts/disconnect/'.$bank_account->id).'" class="btn btn-xs confirmation_link" data-title="." >
																                <i class="fa fa-trash"></i> Disconnect &nbsp;&nbsp;
																            </a>';
	                                                            		}else{
			                                                            	echo '
															            	<a href="'.site_url('admin/bank_accounts/verify/'.$bank_account->id).'" class="btn btn-xs confirmation_link" data-title="" >
	...
	function disable_notifications($group_id = 0){
		if($group_id){
			...
			if($group){
				...
				if($this->groups_m->update($group_id,$input)){
					$this->session->set_flashdata('info','Successfully disabled all group notifications');
					redirect('admin/groups/listing');
				}else{
					$this->session->set_flashdata('error','Could not disable all group notifications');
					redirect('admin/groups/listing');
				}
			}else{
				$this->session->set_flashdata('warning','Could not find group records');
				redirect('admin/groups/listing');
			}
		}else{
			$this->session->set_flashdata('warning','Group ID missing');
			redirect('admin/groups/listing');
		}
	}

	function enable_notifications($group_id = 0){
		if($group_id){
			$group = $this->groups_m->get($group_id);
			if($group){
				$input = array(
				...
				if($this->groups_m->update($group_id,$input)){
					$this->session->set_flashdata('info','Successfully re-enabled all group notifications');
					redirect('admin/groups/listing');
				}else{
					$this->session->set_flashdata('error','Could not re-enabled all group notifications');
					redirect('admin/groups/listing');
				}
			}else{
				$this->session->set_flashdata('warning','Could not find group records');
	...
	function delete($group_id = 0,$confirmation_code = '',$redirect = TRUE){
			...
			if($confirmation_code=="thecheese"){
				...
				$this->investment_groups->backup_group_data($group_id,1,0,$this->user->id,1);
			}else{
	...
	function export_group_data($group_id = 0){
		if($group_id){
			// set_time_limit(0);
   //      	ini_set('memory_limit','4096M');
   //      	ini_set('max_execution_time', 1200);
			$file = $this->_backup_group($group_id,0,1);
			$this->curl->download_file($file);
		}else{
	...
	function import_group(){
		...
		//if(preg_match('/\.local/', $_SERVER['HTTP_HOST'])){
			...
			if($_FILES){				
				...
				if(isset($_FILES['group_file'])){					
					...
					if(preg_match('/zip/', $file['type'])){
						$result = $this->files_uploader->upload('group_file','logs/imported_groups');
						if($result){
							if($id = $this->migrate_m->restore_group(0,$result['full_path'])){
								...
								redirect('admin/groups/search/'.$id);
							}
						}
					}else{
						if($file['tmp_name']){
							$file_path = $file['tmp_name'];
							if($id = $this->migrate_m->restore_group(0,$file_path)){
							//if($id = $this->migrate_m->import_group($file_path)){
								$users = $this->users_m->get_all_users();
							    foreach ($users as $user) {
							      if($this->ion_auth->is_in_group($user->id,2) || $this->ion_auth->is_in_group($user->id,1)){
		...
		// }
		$this->template->title('Import Group')->build('admin/import_group',$this->data);
	}
	...
	function reset_group($group_id = 0,$confirmation_code = '',$redirect = TRUE){
			...
			if($confirmation_code=="thecheese"){
				...
				$tables=$this->db->query("SELECT t.TABLE_NAME AS table_name FROM INFORMATION_SCHEMA.TABLES AS t WHERE t.TABLE_SCHEMA = '".$database."' ")->result_array();    
					$count = 1;
					$ignore_tables = array('equity_bank_transaction_alerts','transaction_alerts','investment_groups','members','bank_accounts','sacco_accounts','mobile_money_accounts','petty_cash_accounts','users_groups');
    ...
    function mark_groups_as_unreconciled(){
    	$this->groups_m->mark_groups_as_unreconciled();
    }
    ...
    function truncate_tables(){
    	if(preg_match('/\.local/', $_SERVER['HTTP_HOST'])){
			...
			$ignore_tables = array(
				'admin_menus',
				'admin_quick_action_menus',
				'banks',
				'bank_branches',
				'bank_menus',
				'billing_packages',
				'billing_packages_plans',
				'countries',
				'currency',
				'email_templates',
				'groups',
			...
			foreach ($tables as $table) {
				if(in_array($table, $ignore_tables)){}else{
    ...
    function login_as_admin($id=0){
		...
		// }else{
		// 	redirect($this->application_settings->protocol.'app.'.$this->application_settings->url.'/group');
		// }
		redirect($this->application_settings->protocol.$this->application_settings->url.'/group');
	}

	function generate_partitioning(){
   		$max_group_id = $this->groups_m->get_group_max_id();
   		$query = " ALTER TABLE statements PARTITION BY RANGE(group_id) ( <br/>";
   		$partition_number = 0;
   		for($i = 1; $i <= $max_group_id; $i++):
   			if(is_int($i/10)){
   				$query .= " PARTITION p".$partition_number." VALUES LESS THAN (".$i."),<br/> ";
   				$partition_number++;
   			}
   		endfor;
   		$query .= " PARTITION p".$partition_number." VALUES LESS THAN MAXVALUE ); ";
   		echo $query;
   }
