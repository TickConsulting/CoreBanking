...
class Bank extends Bank_Controller{
    ...
    function index(){
        $this->template->title('Bank Accounts List')->build('bank/listing',$this->data);
    }
    ...
    public function create(){
        ...
        if($this->form_validation->run())
        {
            ...
            if($id){
                ...
                $this->session->set_flashdata('success', 'Bank Account was successfully added');
                if($this->input->post('new_item')){
                    redirect('bank/bank_accounts/create','refresh');
                }else{
                    if($this->banks_m->is_partner($this->input->post('bank_id'))){
                        redirect('bank/bank_accounts/connect/'.$id.'/'.$this->input->post('account_number'),'refresh');
                    }else{
                        redirect('bank/bank_accounts/listing','refresh');
                    }
                }
            }else{
                $this->session->set_flashdata('error', 'There was an error adding new Bank Account');
                redirect('bank/bank_accounts/create','refresh');
            }
        }
        else
        ...
        if($default_bank){
            $this->template->title('Create '.$default_bank->name.' Bank Account')->build('bank/form',$this->data);
        }else{
            $this->template->title('Create Bank Account')->build('bank/form',$this->data);
        }
    }

    public function connect($id = 0,$account_number = ''){
        $id OR redirect('bank/bank_accounts/listing');
        $post = $this->bank_accounts_m->get_group_bank_account($id);        
        $post OR redirect('bank/bank_accounts/listing');
        $this->data['account'] = $post;
        $post->is_linked != 1 OR redirect('bank/bank_accounts/listing');
        $bank = $this->banks_m->get($post->bank_id); 
        $bank OR redirect('bank/bank_accounts/listing');
        $post = new stdClass();
        $this->data['bank_account_options'] = $this->bank_accounts_m->get_group_unlinked_partner_bank_account_options();
        $this->data['id'] = $id;
        $this->template->title('Connect '.$this->application_settings->application_name.' to your Bank Account')->build('bank/connect',$this->data);
    }
        ...
        if($this->form_validation->run()){
            ...
            if($bank_account = $this->bank_accounts_m->get_group_bank_account($id)){
                if($bank = $this->banks_m->get($bank_account->bank_id)){
                    ...
                    echo 'Could not find Bank';
                }
            }else{
                echo 'Could not find Bank account';
            }
        }else{
    ...
    public function verify_ownership($id = ''){
        ...
        $this->data['id'] = $id;
        $this->data['account'] = $post;
        $this->template->title('Connect Chamasoft to your Bank Account')->build('bank/verify_ownership',$this->data);
    }
    ...
    public function disconnect($id = ''){
        ...
        redirect('bank/bank_accounts/listing');
    }
    ...
    function edit($id=0){
        ...
        if($this->form_validation->run()){
            ...
            if($update){
                $this->session->set_flashdata('success', 'Bank Account was successfully updates');
                if($this->input->post('new_item')){
                    redirect('bank/bank_accounts/create','refresh');
                }else{
                    redirect('bank/bank_accounts/listing','refresh');
                ...
                $this->session->set_flashdata('error', 'There was an error updating Bank Account');
                redirect('bank/bank_accounts/edit/'.$id,'refresh');
            }
        ...
        $this->template->title('Edit Bank Account')->build('bank/form',$this->data);
    }
    ...
    function ajax_get_bank_accounts_listing(){
        ...
        $pagination = create_pagination('bank/bank_accounts/listing/pages', $total_rows,50,5,TRUE);
                        ...
                        echo '
                            <tr>
                                ...
                                if($post->is_default){
                                    ...
                                    echo '
                                    <div class="btn-group">
                                        <a href="'.site_url('bank/bank_accounts/edit/'.$post->id).'" class="btn btn-sm btn-primary">
                                    ...
                                    if($post->is_closed){ 
                                        echo '
                                            <a href="'.site_url('bank/bank_accounts/reopen/'.$post->id).'" class="dropdown-item confirmation_link"><i class="fa fa-play"></i>Reopen</a>
                                        ';
                                    }else{
                                        if($post->active){
                                            echo '
                                                <a href="'.site_url('bank/bank_accounts/hide/'.$post->id).'" class="dropdown-item confirmation_link"><i class="fa fa-eye-slash"></i>Hide</a>
                                            ';
                                            if($post->partner){
                                                if($post->is_verified){ 
                                                    echo '
                                                        <a href="'.site_url('bank/bank_accounts/disconnect/'.$post->id).'" class="dropdown-item confirmation_link"><i class="fa fa-unlock"></i>Disconnect</a>';
                                                }else{
                                                    echo '
                                                    <a href="'.site_url('bank/bank_accounts/connect/'.$post->id).'" class="dropdown-item confirmation_link"><i class="fa fa-check-square"></i>Connect</a>';
                                                }
                                            }
                                        }else{
                                            echo '
                                                <a data-original-title="Activate Bank Account" href="'.site_url('bank/bank_accounts/activate/'.$post->id).'" class="dropdown-item confirmation_link"><i class="la la-check-square-o"></i>Unhide</a>
                                            ';
                                        }
                                        echo '
                                            <a data-original-title="Close Bank Account" href="'.site_url('bank/bank_accounts/close/'.$post->id).'" class="dropdown-item confirmation_link"><i class="fa fa-pause"></i>Close</a>
                                        ';
                                    }
                                    echo '
                                        <a data-original-title="Delete Bank Account" href="'.site_url('bank/sacco_accounts/delete/'.$post->id).'" class="dropdown-item prompt_confirmation_message_link" id="'.$post->id.'"><i class="fa fa-trash"></i>Delete</a>
                                        </div>
                                    </div>';
                                }
                                echo '
