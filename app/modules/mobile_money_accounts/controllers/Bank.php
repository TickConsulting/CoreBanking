...
class Bank extends Bank_Controller{
    ...
    function index(){
        $this->template->title('Mobile Money Cash Accounts List')->build('bank/listing',$this->data);
    }
    ...
    public function create()
    {
        ...
        if($this->form_validation->run())
        {
           ...
           if($id)
           {
                ...
                {
                    redirect('bank/mobile_money_accounts/create','refresh');
                }else
                {
                    redirect('bank/mobile_money_accounts/listing/','refresh');
                }
           }
           else
           {
        ...
        $this->template->title('Create Mobile Money Account Cash Account')->build('bank/form',$this->data);
    }
    ...
    function edit($id=0)
    {
        $id OR redirect('bank/mobile_money_accounts/listing');
        ...
        if(empty($post))
        {
            ...
            redirect('bank/mobile_money_accounts/listing');
            return FALSE;
        }
        ...
        $this->data['id'] = $id;
        $this->data['mobile_money_providers'] = $this->mobile_money_providers_m->get_group_mobile_money_provider_options();
        $this->template->title('Edit Mobile Money Cash Account')->build('bank/form',$this->data);
    }
    ...
    function ajax_get_mobile_money_accounts_listing(){
        ...
        $pagination = create_pagination('bank/mobile_money_accounts/listing/pages', $total_rows,50,5,TRUE);
                        ...
                        echo '
                            <tr class="'.$post->id.'_active_row">
                                ...
                                <td class="actions">';
                                    echo '
                                        <div class="btn-group">
                                            <a href="'.site_url('bank/mobile_money_accounts/edit/'.$post->id).'" class="btn btn-sm btn-primary">
                                    ...
                                    ';
                                        if($post->is_closed){ 
                                            echo '
                                                <a href="'.site_url('bank/mobile_money_accounts/reopen/'.$post->id).'" class="dropdown-item confirmation_link"><i class="fa fa-play"></i>Reopen</a>
                                            ';
                                        }else{
                                            if($post->active){
                                                echo '
                                                    <a href="'.site_url('bank/mobile_money_accounts/hide/'.$post->id).'" class="dropdown-item confirmation_link"><i class="fa fa-eye-slash"></i>Hide</a>
                                                ';
                                            }else{
                                                echo '
                                                    <a data-original-title="Activate Mobile Money Account" href="'.site_url('bank/mobile_money_accounts/activate/'.$post->id).'" class="dropdown-item confirmation_link"><i class="la la-check-square-o"></i>Unhide</a>
                                                ';
                                            }
                                            echo '
                                                <a data-original-title="Close Mobile Money Account" href="'.site_url('bank/mobile_money_accounts/close/'.$post->id).'" class="dropdown-item confirmation_link"><i class="fa fa-pause"></i>Close</a>
                                            ';
                                        }
                                    echo '
                                        <a data-original-title="Delete Mobile Money Account" href="'.site_url('bank/mobile_money_accounts/delete/'.$post->id).'" class="dropdown-item prompt_confirmation_message_link" id="'.$post->id.'"><i class="fa fa-trash"></i>Delete</a>
