<div class="row">
    <div class="col-md-12">
                            ...
                            <?php echo form_open(current_url(),'method="GET" class="filter"');?>
                                <div class="form-body">
                                    <div class="form-group m-form__group">
                                        <label>
                                            <?php echo translate('Cash In Date Range');?>
                                        </label>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="">
                                            <?php
                                            echo form_dropdown('money_market_investments[]',array()+$money_market_investment_options,$this->input->get('money_market_investments'),'class="form-control select2" multiple="multiple" id = "type"  ');
                                            ?>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="btn-group margin-bottom-20 search-button">
            <a href="<?php echo site_url('group/money_market_investments/cash_ins').$query; ?>" class="btn btn-sm btn-primary m-btn m-btn--icon">
                <span>
...
<div id="money_market_investment_cash_ins_listing" class="table-responsive">
...
var get_string = "<?php echo $_SERVER['QUERY_STRING']; ?>";
    ...
    $(window).on('load',function(){
        mApp.block('#money_market_investment_cash_ins_listing',{
            overlayColor: 'white',
            animate: true
        });
        $.ajax({
            type: "GET",
            url: '<?php echo base_url("ajax/money_market_investments/get_money_market_investment_cash_ins_listing/'+first_uri_segment+'/'+second_uri_segment+'?'+get_string+'"); ?>',
            dataType : "html",
                success: function(response) {
                    $('#money_market_investment_cash_ins_listing').html(response);
                    $('.select2').select2({width:"100%"});
                    $('.date-picker').datepicker({autoclose:true});
                    mApp.unblock('#money_market_investment_cash_ins_listing');
                }
            }
        );
    });
