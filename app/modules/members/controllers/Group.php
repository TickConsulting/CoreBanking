...
class Group extends Group_Controller{
    ...
    public function index(){
        $this->template->title(translate('Members'))->build('group/directory');
    }
    ...
    public function invite_members(){
        ...
        $this->template->title(translate('Invite Members'))->build('group/invite_members',$data);
    }

    public function add_members(){
        if(!$this->group->enable_add_members_manually){
            redirect('group/members/invite_members');
        }
        $data = array();
        $posts = $_POST;
        $errors = array();
    ...
    public function ajax_add_member(){
        ...
        if($this->form_validation->run()){
            ...
            if($member_id = $this->group_members->add_member_to_group($this->group,$first_name,$last_name,$phone,$email,$send_invitation_sms,$send_invitation_email,$this->user,$this->member->id,$group_role_id,$middle_name)){
                if($member = $this->members_m->get_group_member($member_id)){
                    $response = array(
                        'status' => 1,
                        'member' => $member,
                        'message' => 'User successfully added',
                        'refer'=>site_url('group/members/listing')
                    );
                }else{
    ...
    public function import_members(){
        ...
        $this->template->title(translate('Import Members'))->build('group/import_members',$data);
    }
    ...
    public function update_members(){
        ...
        $this->template->title('Update Members')->build('group/update_members',$data);
    }
    ...
    public function listing(){
        ...
        $data['group_member_options'] = $this->members_m->get_group_member_with_membership_number_options();
        $this->template->title(translate('List Members'))->build('group/listing',$data);
    }

    public function membership_requests(){
        $data = array();   
        $data['group_member_options'] = $this->members_m->get_group_member_with_membership_number_options();
        $this->template->title(translate('Membership Requests'))->build('group/membership_requests',$data);
    }
    ...
    public function reject_member($member_id=0){
        ...
        // redirect to self registered members listing.
        redirect('group/members/self_registered_members');
    }

    public function directory(){
        $data = array();
        $total_rows = $this->members_m->count_active_group_members();
        $pagination = create_pagination('group/members/directory/pages', $total_rows,25,5,TRUE);
        $data['group_role_options'] = $this->group_roles_m->get_group_role_options();
        ...
        $this->template->title('Member Directory')->build('group/directory',$data);
    }
...
//         }   
    ...
    public function send_invitation($id = 0,$redirect = TRUE){
        $id OR redirect('group/members/listing');
        $post = $this->members_m->get_group_member($id);
        $post OR redirect('group/members/listing');
        $user = $this->ion_auth->get_user($post->user_id);
        $user OR redirect('group/members/listing');
