...
<div id="general_user_details">
    <?php echo form_open_multipart($this->uri->uri_string(), ' class="form_submit m-form m-form--state" role="form" id="edit_members_line"'); ?>
        ...
        <!-- <fieldset class="m--margin-top-10 ">
            <legend class="m-3 p-2"><?php echo translate('Membership Details');?></legend>
            <div class="form-group m-form__group row pt-0 m--padding-10">
                <div class="col-sm-4 m-form__group-sub m-input--air">
                    <label><?php echo translate('Membership Number');?><span class="required">*</span></label>

                    <?php echo form_input('membership_number',$this->input->post('membership_number')?$this->input->post('membership_number'):$post->membership_number,'class="form-control name m-input--air" placeholder="Membership Number"'); ?>
                </div>
                <div class="col-sm-4 m-form__group-sub m-input--air">
                    <label><?php echo translate('Member Role');?></label>
                    <?php echo form_dropdown('group_role_id',array(''=>'Member')+$group_role_options+array('0'=>"Add Group Role"),$this->input->post('group_role_id')?$this->input->post('group_role_id'):$post->group_role_id,'class="form-control m-select2 group_role" id = "group_role"  ') ?>
                ...
                <?php }else{ ?>
                    <div class="col-sm-4 m-form__group-sub m-input--air">
                        <label><?php echo translate('Access Level');?></label>
                        <?php echo form_dropdown('is_admin',$this->access_levels,$this->input->post('is_admin')?$this->input->post('is_admin'):$post->is_admin,'class="form-control m-select2" placeholder="Access Level" ') ?>                                             
                    </div>
                <?php } ?>
                <?php echo form_hidden('user_id',$post->user_id); ?>
            </div>
                ...
                <div class="col-sm-12 m-form__group-sub m-input--air">
                    <div style="background:#fff!important;border: none;display: none;" class="no_records">
                        <div class="alert m-alert--outline alert-metal text-center">
                            <h4 class="block"><?php echo translate('Information');?>! <?php echo translate('No records to display');?></h4>
                            <p>
                                <?php echo translate('There is no next of kin registered for this member'); ?>
                            </p>
                        </div>
                    </div>
...
<div class="modal fade" id="add_new_role" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <?php echo form_open($this->uri->uri_string(), ' class="form_submit m-form m-form--state" role="form" id="add_new_member_role_form"'); ?>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary " id="close_add_role" data-dismiss="modal">Close</button>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#avatar').on('change',function(){
            $('.m-card-profile__pic-wrapper').html('')
            var avatar_img = $('#avatar').val();            
            $('.m-card-profile__pic-wrapper').html('<img src="<?php echo is_file(FCPATH.'uploads/groups/')?site_url('uploads/groups/'.$post->avatar):site_url('templates/admin_themes/admin/img/no_image.png'); ?>" alt="" class="avatar_profile" />')
            alert('me')
