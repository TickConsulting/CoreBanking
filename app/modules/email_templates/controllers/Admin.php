...
*/
    ...
    public function edit($id=0)
    {
        ...
        $post or redirect('admin/email_templates/listing');
        $this->form_validation->set_rules($this->validation_rules);
        if ($this->form_validation->run()){
    ...
    function test_mail($id=0){
        $id OR redirect('admin/email_templates/listing');
        $post = $this->email_templates_m->get($id);
        if($post){
            $email_array = array(
            ...
            if($this->emails_manager->send_email_via_postmark_api($email_array)){
                $this->session->set_flashdata('success','Email template successfully sent');
            }else{  
                $this->session->set_flashdata('error','Could not send the requested email template');
            }
        }else{
            $this->session->set_flashdata('error','Could not find the requested email template');
        }
        redirect('admin/email_templates/listing');
    }
