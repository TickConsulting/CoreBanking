...
class Admin extends Admin_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('member_settings_menus_m');
    }

    function menu()
    {
       //$this->member_settings_menus_m->great_great_grand_child_is_active('',uri_string());
       $data['posts'] = array();
    ...
    function index()
    {
        /*$this->member_settings_menus_m->generate_side_bar_menu();die;
        $this->template->title('Settings Menus')->build('admin/index');*/
    }

    function delete($id = 0,$redirect= TRUE)
    {
        $id OR redirect('admin/member_settings_menus/listing');

        $post = $this->member_settings_menus_m->get($id);

        if(!$post)
        {
            $this->session->set_flashdata('error','Sorry, the Admin Menu does not exist');
            redirect('admin/member_settings_menus/listing');
        }

        $id = $this->member_settings_menus_m->delete($post->id);
    ...
    function hide($id=0,$redirect=TRUE)
    {
        ...
        $id = $this->member_settings_menus_m->update($post->id,array
            (
                'active'=>NULL,
                'modified_on' => time(),
    ...
    function listing()
    {
        $this->data['posts'] = $this->member_settings_menus_m->get_parent_links();
        $this->data['side_bar_menu_options'] = $this->member_settings_menus_m->get_options();
    ...
    function create()
    {
        ...
        if($this->form_validation->run())
        {
            ...
            $id = $this->member_settings_menus_m->insert($data);
            if($id)
            {
                $this->session->set_flashdata('success','Menu Item Created Successfully.');
                if($this->input->post('new_item'))
                {
                    redirect('admin/member_settings_menus/create','refresh');
                }
                else
                {
                    redirect('admin/member_settings_menus/edit/'.$id);
                }
                
            }
            else{
                $this->session->set_flashdata('error','Menu Item could not be Created.');
                redirect('admin/member_settings_menus/create');
            }
        }
        else
        {
            foreach ($this->rules as $key => $field) {
                 ...
                 $menu->$field['field'] = set_value($field['field']);
            }
        }

        $this->data['menus'] = $this->member_settings_menus_m->get_options();
        $this->data['tile_colors']=$this->tile_colors;
        $this->data['tile_sizes']=$this->tile_sizes;
        $this->data['menu'] = $menu;
        $this->data['post'] = $post;
    ...
    function edit($id=0)
    {
        ...
        $menu = $this->member_settings_menus_m->get($id);
        if(!$menu)
        {
            $this->session->set_flashdata('error','Sorry, the Admin Menu does not exist');
            redirect('admin/member_settings_menus/listing');
        ...
        if($this->form_validation->run())
        {
            ...
            $update = $this->member_settings_menus_m->update($id,$data);
            if($update)
            {
                ...
                {
                    redirect('admin/member_settings_menus/listing','refresh');
                }
            }
            else
            {
    ...
    function ajax_sort_update()
    {
        ...
        for($i=0;$i<count($data);$i++){
            $this->member_settings_menus_m->update($data[$i]->id,array(
                'position'=>$i,
                'modified_on' => time(),
                'modified_by' => $this->ion_auth->get_user()->id,
            ));
    ...
    private function _children($pt,$parent_id,$position){
        ...
        $this->member_settings_menus_m->update($pt->id,array(
            'position'=>$position,
            'parent_id'=>$parent_id,
            'modified_on' => time(),
            'modified_by' => $this->ion_auth->get_user()->id,
