...
class Settings_menus_m extends MY_Model {

	protected $_table = 'settings_menus';
	...
	**/
	...
	{
		$this->select_all_secure('settings_menus');
		$this->db->where('id',$id);
		return $this->db->get('settings_menus')->row();
	}

	function insert($input = array(),$skip_value = FALSE)
	{
		return $this->insert_secure_data('settings_menus',$input);
	}
	...
	{ 
		...
		$res = $this->db->get('settings_menus')->row();
	...
	function update($id,$input,$val=FALSE){
    	return $this->update_secure_data($id,'settings_menus',$input);
    }

	function get_all()
	{
		$this->select_all_secure('settings_menus');
		return $this->db->get('settings_menus')->result();
	}

	function count_all_active()
	{
		return $this->db->count_all_results('settings_menus');
	}

	function get_options(){
		$arr = array();
		$this->select_all_secure('settings_menus');
		$menus = $this->db->get('settings_menus')->result();
	...
	function has_children($parent_id = 0){
		$this->db->where($this->dx('parent_id').' = '.$parent_id,NULL,FALSE);
		return $this->db->count_all_results('settings_menus')>0?TRUE:FALSE;	
	}
		...
		if(!empty($links)){
			...
			foreach($links as $link):
				echo '<li class="dd-item" data-id="'.$link->id.'">
	                        ...
	                        echo '</div>';
	                        $this->settings_menus_m->display_children($link->id); 
	            echo '</li>';
	        endforeach;
			echo '</ol>';
		}	
	...
	function child_is_active($parent_id,$child_link_url){
		...
		if($child_link_url){
			...
			$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
			$menu = $this->db->get('settings_menus')->row();
			if($menu){
	...
	function grand_child_is_active($parent_id,$grand_child_link_url){
		...
		if($grand_child_link_url){
			...
			$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
			$parent_menu = $this->db->get('settings_menus')->row();	
			if($parent_menu){
				$this->db->select(array($this->dx('parent_id').' as parent_id '));
	...
	function great_grand_child_is_active($parent_id,$great_grand_child_link_url){
		...
		if($great_grand_child_link_url)
		{
			...
			$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
			$grand_child_menu = $this->db->get('settings_menus')->row();
			if($grand_child_menu)
			{
				$this->db->select(array($this->dx('parent_id').' as parent_id '));
	...
	function generate_side_bar_menu(){
		$parent_links = $this->get_active_parent_links();
		if($parent_links){
			echo '<ul class="feeds list-items">';
				...
				echo '  <li>	
							<div class="col1">
	                            <div class="cont">
	                                ...
	                                <div class="cont-col2">
	                                    <div class="desc"> 
	                                		<h5 class="setting-headings">'.$link->name.'</h5>
	                                    	'.$link->description.'
	                                    	<div class="margin-top-10">
		                                        <span class="label call-to-action label-sm label-success "><a href="'.site_url($link->url).'"> '.$link->call_to_action.'
	...
	function get_menu_by_link_url($link_url='')
	{
		...
		$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
		return $menu = $this->db->get('settings_menus')->row();
	}
	...
	function get_search_options(){
		...
		$this->db->where(" ( 
				CONVERT(" . $this->dx('name') . " USING 'latin1')  like '%" . $this->escape_str($query) . "%' OR 
				CONVERT(" . $this->dx('description') . " USING 'latin1')  like '%" . $this->escape_str($query) . "%' 
			)", NULL, FALSE);
		$this->db->order_by($this->dx('name'),'DESC',FALSE);
		$settings_menus = $this->db->get('settings_menus')->result();

		$result->total_count = count($settings_menus);
		$result->incomplete_results = false;
		$result->items = $settings_menus;
