...
class Admin extends Admin_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('admin_menus_m');
    }

    function menu()
    {
       //$this->admin_menus_m->great_great_grand_child_is_active('',uri_string());
       $data['posts'] = array();
    ...
    function index()
    {
        /*$this->admin_menus_m->generate_side_bar_menu();die;
        $this->template->title('Admin Menus')->build('admin/index');*/
    }

    function delete($id = 0,$redirect= TRUE)
    {
        $id OR redirect('admin/admin_menus/listing');
        ...
        if(!$post)
        {
            ...
            redirect('admin/admin_menus/listing');
        }

        $id = $this->admin_menus_m->delete($post->id);
    ...
    function hide($id=0,$redirect=TRUE)
    {
        ...
        $id = $this->admin_menus_m->update($post->id,array
            (
                'active'=>NULL,
                'modified_on' => time(),
    ...
    function listing()
    {
        $this->data['posts'] = $this->admin_menus_m->get_parent_links();
        $this->data['side_bar_menu_options'] = $this->admin_menus_m->get_options();
        $this->data['tile_sizes'] = $this->tile_sizes;
        $this->data['tile_colors'] = $this->tile_colors;
        $this->template->title('Admin Menus')->build('admin/listing',$this->data);
    }
    ...
    function create()
    {
        ...
        $this->data['menus'] = $this->admin_menus_m->get_options();
        $this->data['tile_colors']=$this->tile_colors;
        $this->data['tile_sizes']=$this->tile_sizes;
        $this->data['post'] = $post;
        $this->template->title('Admin Menus Form')->build('admin/form',$this->data);
    }
    ...
    function edit($id=0)
    {
        ...
        if($this->form_validation->run())
        {
            ...
            $update = $this->admin_menus_m->update($id,$data);
            if($update)
            {
                ...
                {
                    redirect('admin/admin_menus/listing','refresh');
                }
            }
            else
            {
    ...
    function ajax_sort_update()
    {
        ...
        for($i=0;$i<count($data);$i++){
            $this->admin_menus_m->update($data[$i]->id,array(
                'position'=>$i,
                'modified_on' => time(),
                'modified_by' => $this->ion_auth->get_user()->id,
            ));
    ...
    private function _children($pt,$parent_id,$position){
        ...
        $this->admin_menus_m->update($pt->id,array(
            'position'=>$position,
            'parent_id'=>$parent_id,
            'modified_on' => time(),
            'modified_by' => $this->ion_auth->get_user()->id,
