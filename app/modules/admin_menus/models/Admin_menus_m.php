...
class Admin_menus_m extends MY_Model {

	protected $_table = 'admin_menus';
	...
	**/
	...
	{
		$this->select_all_secure('admin_menus');
		$this->db->where('id',$id);
		return $this->db->get('admin_menus')->row();
	}

	function insert($input = array(),$skip_value = FALSE)
	{
		return $this->insert_secure_data('admin_menus',$input);
	}
	...
	{ 
		...
		$res = $this->db->get('admin_menus')->row();
	...
	function update($id,$input,$val=FALSE){
    	return $this->update_secure_data($id,'admin_menus',$input);
    }

	function get_all()
	{
		$this->select_all_secure('admin_menus');
		return $this->db->get('admin_menus')->result();
	}

	function count_all_active()
	{
		return $this->db->count_all_results('admin_menus');
	}

	function get_options(){
		$arr = array();
		$this->select_all_secure('admin_menus');
		$menus = $this->db->get('admin_menus')->result();
	...
	function has_children($parent_id = 0){
		$this->db->where($this->dx('parent_id').' = '.$parent_id,NULL,FALSE);
		return $this->db->count_all_results('admin_menus')>0?TRUE:FALSE;	
	}
		...
		if(!empty($links)){
			...
			foreach($links as $link):
				echo '<li class="dd-item" data-id="'.$link->id.'">
	                        ...
	                        echo '</div>';
	                        $this->admin_menus_m->display_children($link->id); 
	            echo '</li>';
	        endforeach;
			echo '</ol>';
		}	
	...
	function child_is_active($parent_id,$child_link_url){
		...
		if($child_link_url){
			...
			$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
			$menu = $this->db->get('admin_menus')->row();
			if($menu){
	...
	function grand_child_is_active($parent_id,$grand_child_link_url){
		...
		if($grand_child_link_url){
			...
			$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
			$parent_menu = $this->db->get('admin_menus')->row();	
			if($parent_menu){
				$this->db->select(array($this->dx('parent_id').' as parent_id '));
	...
	function great_grand_child_is_active($parent_id,$great_grand_child_link_url){
		...
		if($great_grand_child_link_url)
		{
			...
			$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
			$grand_child_menu = $this->db->get('admin_menus')->row();
			if($grand_child_menu)
			{
				$this->db->select(array($this->dx('parent_id').' as parent_id '));
	...
	function get_menu_by_link_url($link_url='')
	{
		...
		$this->db->where($this->dx('active').' = "1" ',NULL,FALSE);
		return $menu = $this->db->get('admin_menus')->row();
	}
