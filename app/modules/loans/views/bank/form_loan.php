    ...
    <fieldset>
        ...
        <div class="form-group m-form__group row pt-0 m--padding-10">
            <div class="col-sm-12 m-form__group-sub m-input--air">
                ...
                <?php echo form_hidden('loan_repayment_period_type',$id);?>
            </div>
        </div>
        <div class="form-group m-form__group row pt-0 m--padding-10" id="member_to_amount_holder" style="display: none;">
            ...
            <div class="col-sm-6 m-form__group-sub m-input--air " id="member_ids">
                <label><?php echo translate('Select Members to create Loan');?><span class="required">*</span></label>
                <?php echo form_dropdown('member_id',array(''=>translate('Select Member to Create Loan'))+translate($this->active_group_member_options),$this->input->post('member_id')?:$post->member_id?:'',' class="form-control m-input m-select2 member_id" id="member_id" ');?>
            </div>
        ...
        <div class="form-group m-form__group row pt-0 m--padding-10" id="disbursement_account_holder" style="display: none;">
            <div class="col-sm-12 m-form__group-sub m-input--air">
                ...
                <?php echo form_dropdown('account_id',array(''=>translate('Select Disbursment Account'))+translate($active_accounts),$post->account_id,' class="form-control m-input m-select2 account_id" id="account_id"');?>
            </div>
        </div>
        <div class="form-group m-form__group row pt-0 m--padding-10" id="disbursement_date_holder" style="margin-left: 0px;margin-right: 0px;display: none">
            ...
            <div class="input-group ">
                ...
                <?php echo form_input('disbursement_date',$this->input->post('disbursement_date')?timestamp_to_datepicker(strtotime($this->input->post('disbursement_date'))):timestamp_to_datepicker($post->disbursement_date),'class="form-control m-input m-input--air date-picker" readonly data-date-end-date="0d" data-date-format="dd-mm-yyyy" data-date-viewmode="years" id="disbursement_date"');?>
            </div>
        </div>
       ...
       <!--  <div class="form-group m-form__group row pt-0 m--padding-10" id="disbursement_option_holder" style="display: none">
        ...
        <div class="form-group m-form__group row pt-0 m--padding-10" id="repayment_period_holder" style="display: none;">
            <div class="col-md-12 m-form__group-sub m-input--air">
                ...
                <?php echo form_input('repayment_period',$this->input->post('repayment_period')?:$post->repayment_period,'  class="form-control currency m-input--air" placeholder="Loan repayment period" id="repayment_period" '); ?>
            </div>
        </div>
        ...
        <!-- <div class="form-group m-form__group row pt-0 m--padding-10" id="gurantors_period_holder" style="display: none;">
            <div class="col-md-12 m-form__group-sub m-input--air">
                ...
                <?php echo form_dropdown('gurantors_id[]',translate($this->active_group_member_options),$this->input->post('gurantors_id')?:$post->gurantors_id?:'',' class="form-control m-input m-select2 gurantors_id" multiple="multiple" placeholder="--Select Gurantors--" id="gurantors_id"');?>
            </div>
        </div> -->
...
<script type="text/javascript">
    ...
    $(document).ready(function(){ 
        ...
        $('#create_member_loan_form').on('submit',function(e){
            ...
            var resultMembers = $.parseJSON(JSON.stringify('<?php echo xss_clean_input(json_encode($this->active_group_member_options))?>'));
            ...
            var resultMobileMoneyAccounts = $.parseJSON(JSON.stringify('<?php echo xss_clean_input(json_encode($mobile_money_account_recipients))?>'));
        ...
        $('#loan_btn_amortization').on('click',function(e){
            ...
            var loan_repayment_period_type = $('input[name="loan_repayment_period_type"]').val();
            ...
            if(loan_repayment_period_type== "2"){
                if(repayment_period == ""){
                    $('#repayment_period').parent().addClass('has-danger').append('<div class="form-control-feedback">Please enter loan repaymennt period</div>');
                    mApp.unblock('#create_member_loan_form');
                    $(this).removeClass("m-loader m-loader--right m-loader--light").attr("disabled", false)
                    form_error = true;
                }
        ...
        $('#submit_application_button').on('click',function(e){
            $(this).addClass("m-loader m-loader--right m-loader--light").attr("disabled", !0)
            var url = '<?php echo base_url("ajax/loans/create"); ?>';
            <?php if($id){?>
                var url = '<?php echo base_url("ajax/loans/edit"); ?>';
            <?php }?>
            ...
            success: function(response){
                if(isJson(response)){
                    ...
                    if(result){
                        ...
                        $('input[name="loan_repayment_period_type"]').val(result.loan_repayment_period_type);
