     ...
     <table class="table table-striped table-bordered table-hover table-header-fixed table-condensed table-searchable">
        <thead>
            <tr>
                ...
                <th class='text-right'>
                    Amount (<?php echo $this->group_currency; ?>)
                </th>  
                <th>
                    <?php
        ...
        <tbody>
            <?php $i = $this->uri->segment(5, 0); foreach($posts as $post): ?>
                <tr>
                    ...
                    <td><?php echo $i+1;?></td>
                    <td><?php echo $this->active_group_member_options[$post->member_id];?></td>
                    <td>
                    ...
                    <td>
                        <?php if($post->status <3){?>
                            <a href="<?php echo site_url('/group/loans/reveiw_applicaion/'.$post->id)?>" class="btn btn-xs default">
                            <i class="icon-eye"></i> Review &nbsp;&nbsp; 
                            </a>
                        ...
                        <?php }else{?>
                            <a href="<?php echo site_url('/group/loans/redisburse_application/'.$post->id)?>" class="btn btn-xs btn-primary confirmation_link">
                            <i class="icon-pencil"></i> Redisburse Application &nbsp;&nbsp; 
                            </a>
                        <?php } }?>
                    </td>
                </tr>
