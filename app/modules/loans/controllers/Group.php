...
class Group extends Group_Controller{
    ...
    function create(){
            ...
            if($enable_loan_processing_fee){
                ...
                if($id){
                    ...
                    $this->notifications->create(
                        ...
                        'group/loans/view_installments/'.$id,
                        3
                    );
                ...
                if($this->input->post('new_item')){
                    redirect('group/loans/create');
                }else{
                    redirect('group/loans/view_installments/'.$id,'refresh');
                }
            }else{
        ...
        if($loan_types){
            if($this->application_settings->enable_online_disbursement){
                $this->template->title(translate('Create Member Loan'))->build('group/form',$this->data);
            }else{
                
                $this->template->title(translate('Create Member Loan'))->build('group/form_loan',$this->data);
            }
        }else{
            $this->template->title(translate('Create Member Loan'))->build('group/no_loan_type_form',$this->data);
        }
    }
    ...
    function edit_loan($id=0){
        ...
        $this->template->title('Edit Loan Request ')->build('group/apply',$data);
     }
    ...
    function edit($id = 0){ 
        ...
        if(!$post){
            ...
            redirect('group/loans/listing','refresh');
            die;
        }
        ...
        if($this->form_validation->run()){
            ...
            if($update){
                ...
                if($this->input->post('new_item')){
                    ...
                    redirect('group/loans/view_installments/'.$update);
                }
            }else{
                redirect('group/loans/edit/'.$id);
            }
        }else{
        ...
        if($loan_types){
            if($this->application_settings->enable_online_disbursement){
                $this->template->title(translate('Edit Member Loan'))->build('group/form',$this->data);
            }else{
                $this->template->title(translate('Edit Member Loan'))->build('group/form_loan',$this->data);
            }
        }else{
            $this->template->title(translate('Edit Member Loan'))->build('group/no_loan_type_form',$this->data);
        }
        // if($this->application_settings->enable_online_disbursement){
        //     $this->template->title(translate('Edit Member Loan'))->build('group/form',$this->data);
        // }else{
    ...
    function index(){
        ...
        $this->template->title(translate('Group Loans'))->build('group/index',$this->data);
    }
    ...
    function listing(){
        ...
        $this->data['from'] = $from;
        $this->data['to'] = $to;
        $this->template->title(translate('List Member Loans'))->build('group/listing',$this->data);
    }

    function loan_repayments(){
        $total_rows = $this->loan_repayments_m->count_all_group_loan_repayments();
        $pagination = create_pagination('group/loans/loan_repayments/pages', $total_rows,50,5,TRUE);
        ...
        $this->template->title('List Member Loans')->build('group/loan_repayments_listing',$this->data);
    }
    ...
    function pending_member_loan(){
        ...
        $this->template->title('List Pending Member Loans')->build('group/pending_loan_listing',$this->data);  
    }
    ...
    function view_installments($id=0,$generate_pdf=FALSE)
    {
        ...
        if($generate_pdf==TRUE){
            ...
            $html = $this->load->view('group/view_installments',$this->data,TRUE);
            ...
            $this->template->title($loan->first_name.' '.$loan->last_name.' '.$type.' Installments')->build('group/view_installments',$this->data);
        }
    }
    ...
    function reveiw_applicaion($id=0){
        ...
        $this->template->set_layout('default_full_width.html')->title('Review Loan')->build('group/review_form',$this->data);
    }
    ...
    function mark_as_a_bad_loan($id = 0){
        ...
        $loan OR redirect('group/loans/listing');
        $input = array(
            'is_a_bad_loan' => 1,
            'modified_by' => $this->user->id,
            'modified_on' => time(),
