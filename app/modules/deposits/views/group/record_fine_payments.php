<?php echo form_open($this->uri->uri_string(),'class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed form_submit fine_payments_form m-form--state" role="form"'); ?>
    <span class="error"></span>
    <div class="table-responsive">
        <table class="table table-condensed fine-table multiple_payment_entries">
            <thead>
                <tr> 
                    <th width="1%">
                        #
            ...
            <tbody id='append-place-holder'>
                <tr>
                    ...
                    <td>
                        <span class="m-select2-sm m-input--air">
                            <?php echo form_dropdown('fine_categories[0]',array(''=>translate('Select fine categories'))+$fine_category_options+array('0'=>"Add Fine Category"),'',' class="form-control m-select2 fine_category" ');?>
                        </span>
                        <br>
                    ...
                    <td>
                        <span class="m-select2-sm m-input--air">
                            <?php echo form_dropdown('deposit_methods[0]',array(''=>translate('Select deposit method'))+$deposit_method_options,'',' class="form-control m-input m-select2 deposit_method" ');?>
                        </span>
                    </td>
                    <td>
            ...
            <div class="col-md-12">
                <span class="float-lg-right float-md-left float-sm-left float-xl-right">
                    <button class="btn btn-primary m-btn m-btn--custom m-btn--icon btn-sm submit_form_button" id="" type="submit">
                       <?php echo translate('Record Fine Payments');?>                              
                    </button>
                    &nbsp;&nbsp;
    ...
    <table>
        <tbody>
            <tr>
                ...
                <td>
                    <span class="m-select2-sm m-input--air">
                        <?php echo form_dropdown('fine_categories[0]',array(''=>translate('Select fine categories'))+$fine_category_options+array('0'=>"Add Fine Category"),'',' class="form-control m-select2-append fine_category" ');?>
                    </span>
                    <br>
                    <a href="javascript:;" class=" btn btn-sm m-btn--square btn-default btn-xs inline-table-button add_deposit_description" id="" style="margin-top:2px; width: 100%;">
                        <i class="la la-plus"></i>
...
<script>
    $(document).ready(function(){
        ...
        //add fine category modal close eventt
        $('#add_fine_category_popup').on('hidden.bs.modal', function () {
            $(':input','#add_fine_category_popup')
                .val('')
                .prop('checked',false)
                .removeAttr('selected')
                .trigger('change');
            console.log('add fine category modal close eventt');
        ...
        $(document).on('change','select.fine_category',function(){
            ...
            if($(this).val()==''){
                ...
                if($(this).val()=='0'){
                    ...
                    $(this).val("").trigger('change');
                    $('#add_fine_category_popup .select2-append').select2({
                        escapeMarkup: function (markup) {
                            return markup;
                        }
                    });
        ...
        $(document).on('change','select.deposit_method',function(){
            if($(this).val()==''){
                $(this).parent().parent().addClass('has-danger');
            }else{
                $(this).parent().parent().removeClass('has-danger');
        ...
        $('#add-new-line').on('click',function(){
            ...
            $('.fine-table .m-select2-append').select2({
        ...
        $('.fine-table').on('click','a.remove-line',function(event){
        ...
        $(document).on('changeDate','.fine-table input.deposit_date',function(){
            if($(this).val()==''){
                $(this).parent().addClass('has-danger');
            }else{
                $(this).parent().removeClass('has-danger');
        ...
        $('.fine_payments_form').on('submit',function(e){
            e.preventDefault();
            mApp.block('.fine_payments_form', {
                overlayColor: 'grey',
                animate: true,
                type: 'loader',
                state: 'primary',
            ...
            $('.fine_payments_form .submit_form_button').addClass('m-loader m-loader--light m-loader--left').attr('disabled',true);
            if(validate_fine_payments_form()){
                var form = $('.fine_payments_form');
                $.ajax({
                    type: "POST",
                    url: '<?php echo base_url("ajax/deposits/record_fine_payments"); ?>',
                    data: form.serialize(),
                    success: function(data) {
                        ...
                        if(response.status == 1){
                            ...
                            $('.fine_payments_form .error').html('<div class="alert alert-danger alert-dismissible fade show" role="alert"><button type="button" class="close" data-dismiss="alert" aria-label="Close"></button><strong>Sorry!</strong>'+response.message+'</div>');
                        }
                        mApp.unblock('.fine_payments_form');
                        $('.fine_payments_form .submit_form_button').removeClass('m-loader m-loader--light m-loader--left').attr('disabled',false);
                    }
                });

            }else{
                $('.fine_payments_form .error').html('<div class="alert alert-danger alert-dismissible fade show" role="alert"><button type="button" class="close" data-dismiss="alert" aria-label="Close"></button><strong>Sorry!</strong> There are errors on the form, please review the highlighted fields and try submitting again.</div>');
                mApp.unblock('.fine_payments_form');
                $('.fine_payments_form .submit_form_button').removeClass('m-loader m-loader--light m-loader--left').attr('disabled',false);
            }
                ...
                success: function(response) {
                    if(isJson(response)){
                        ...
                        if(data.status == 1){
                            ...
                            $('.fine-table select[name="accounts['+current_row+']"]').val("bank-"+data.bank_account.id).trigger('change');
                ...
                success: function(response) {
                    if(isJson(response)){
                        ...
                        if(data.status == 1){
                            ...
                            $('.fine-table select[name="accounts['+current_row+']"]').val("sacco-"+data.sacco_account.id).trigger('change');
                            $('#create_new_account_pop_up .close').trigger('click');
                ...
                success: function(response) {
                    if(isJson(response)){
                        ...
                        if(data.status == 1){
                            ...
                            $('.fine-table select[name="accounts['+current_row+']"]').val("mobile-"+data.mobile_money_account.id).trigger('change');
                            $('#create_new_account_pop_up .close').trigger('click');
                ...
                success: function(response) {
                    if(isJson(response)){
                        ...
                        if(data.status == 1){
                            ...
                            $('.fine-table select[name="accounts['+current_row+']"]').val("petty-"+data.petty_cash_account.id).trigger('change');
                            $('#create_new_account_pop_up .close').trigger('click');
                ...
                success: function(response) {
                    if(isJson(response)){
                        ...
                        if(data.status == 1){
                            ...
                            $('.fine-table select[name="members['+current_row+']"]').val(data.member.id).trigger('change');
                ...
                success: function(response) {
                    if(isJson(response)){
                        ...
                        if(data.status == 1){
                            ...
                            $('.fine-table select[name="fine_categories['+current_row+']"]').val('fine_category-'+data.fine_category.id).trigger('change');
    ...
    $(window).on('load',function() {
        ...
        $('.fine-table .fine_category').select2({
        ...
        $(".fine-table .deposit_method").select2({
            placeholder:{
                id: '-1',
                text: "--Select option--",
            }, 
    ...
    function validate_fine_payments_form(){
        var entries_are_valid = true;
        $('.fine-table input.deposit_date').each(function(){
            if($(this).val()==''){
                $(this).parent().addClass('has-danger');
                entries_are_valid = false;
            }else{
        ...
        $('.fine-table select.member').each(function(){
            if($(this).val()==''){
                $(this).parent().parent().addClass('has-danger');
                entries_are_valid = false;
            }else{
        ...
        $('.fine-table select.fine_category').each(function(){
            if($(this).val()==''){
                $(this).parent().parent().addClass('has-danger');
                entries_are_valid = false;
            }else{
        ...
        $('.fine-table select.deposit_method').each(function(){
            if($(this).val()==''){
                $(this).parent().parent().addClass('has-danger');
                entries_are_valid = false;
            }else{
        ...
        $('.fine-table input.amount').each(function(){
            if($(this).val()==''){
                $(this).parent().addClass('has-danger');
                entries_are_valid = false;
            }else{
        ...
        $('.fine-table select.account').each(function(){
            if($(this).val()==''){
                $(this).parent().addClass('has-danger');
                entries_are_valid = false;
            }else{
