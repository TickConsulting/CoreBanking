<?php echo form_open($this->uri->uri_string(),'class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed form_submit loan_repayments_form m-form--state" role="form"'); ?>
                ...
                <tr>
                    ...
                    <td>
                        <span class="m-select2-sm m-input--air change-loan">
                            <?php echo form_dropdown('loans[0]',array(''=>translate('Select Member First'))+$loans_options,'',' class="form-control m-input m-select2 loan" readonly ');?>
                        </span>
                        <br>
    ...
    <table>
        <tbody>
            <tr>
                ...
                <td>
                   <span class="m-select2-sm m-input--air change-loan">
                        <?php echo form_dropdown('loans[0]',array(''=>translate('Select Member First'))+$loans_options,'',' class="form-control m-input m-select2-append loan" readonly ');?>
                    </span>
                    <br>
                    <a href="javascript:;" class=" btn btn-sm m-btn--square btn-default btn-xs inline-table-button add_deposit_description" id="" style="margin-top:2px; width: 100%;">
                        <i class="la la-plus"></i>
...
<script>
    $(document).ready(function(){
            ...
            if($(this).val()==''){
                ...
                if($(this).val()=='0'){
                    ...
                    $.ajax({
                        ...
                        url: url,
                        dataType: "html",
                        data: {'member_id': member_id , 'attribute':attribute,'loan_id':loan_id},
                        success: function(res) 
                        {
                            member.parent().parent().parent().find('.change-loan').html(res);
                            member.parent().parent().parent().find('.loan').select2({
        ...
        <?php if($this->input->get_post('member_id')){?>
            $('.table-multiple-items select.member').trigger('change');
        <?php }?>
        ...
        $(document).on('click','#add_loan',function(){
            ...
            var member_id = $('select[name="members['+current_row+']"]').val();
            $('#modal').on('shown.bs.modal', function(){
                $(document).find('#submit_form select[name="member_id"]').val(member_id).trigger('change').prop('disabled',true);
                $(document).find('#submit_form input[name="member_id"]').val(member_id);
            });
        });
