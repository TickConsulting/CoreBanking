    ...
    <div class="col-md-3 col-xs-6">
        ...
        <div class="widget-thumb widget-bg-color-white text-uppercase margin-bottom-5 bordered">
            ...
            <div class="widget-thumb-wrap">
                ...
                <div class="widget-thumb-body">
                    <span class="widget-thumb-subtitle"><?php echo $this->group_currency;?></span>
                    <span class="widget-thumb-body-stat" data-counter="counterup" data-value="<?php echo number_to_currency($total_group_member_contribution_arrears);?>"><?php echo number_to_currency($total_group_member_contribution_arrears);?></span>
                </div>
            </div>
        </div>
        <!-- END WIDGET THUMB -->
...
<a href="#gSubscrModal" style="margin-left:5px;margin-bottom:10px;" data-toggle="modal" role="button" class="btn btn-circle hidden btn-sm green d-none" data-backdrop="static" id="add_payment_options" data-keyboard="false">Pay Now <i class="fa fa-chevron-circle-right"></i></a>
...
<script type="text/javascript">
    ...
    function calculateConvenienceCharge(form_data,content,message=''){
        $.ajax({
            type: "POST",
            url: '<?php echo base_url("ajax/deposits/calculate_convenience_charge"); ?>',
            data: form_data,
        ...
        var t = function (redirect,modal) {
            $(document).on('click',".btn#make_payment_form_button",function (t) {
                ...
                (e.addClass("m-loader m-loader--right m-loader--light").attr("disabled", !0),a.ajaxSubmit({
                    type: "POST",
                    url: base_url+"/ajax/deposits/make_payment_validation",
        ...
        var t = function (redirect,modal) {
            $(document).on('click',".btn#mpesa_payment_button",function (t) {
                ...
                (e.addClass("m-loader m-loader--right m-loader--light").attr("disabled", !0),$.post({
                    type: "POST",
                    url: base_url+"/ajax/deposits/initiate_member_payments",
                    data: form_data,
                    success: function (t, i, n, r) {
                        if(isJson(t)){
                            response = $.parseJSON(t);
