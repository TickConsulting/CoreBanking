<?php if(!empty($unreconciled_withdrawals)){ ?>
    <?php echo form_open('group/transaction_alerts/action', ' id="form"  class="form-horizontal"'); ?> 
        <div class='table-responsive'>
            <table class="table table-bordered table-condensed table-striped table-hover table-header-fixed table-searchable">
                ...
                <tbody>
                    <?php $count=1; foreach($unreconciled_withdrawals as $unreconciled_withdrawal): ?>
                    	<tr  id="reconciled_deposit_row_<?php echo $unreconciled_withdrawal->id; ?>">
                    		<td><input name='action_to[]' type="checkbox" class="checkboxes" value="<?php echo $unreconciled_withdrawal->id; ?>" /></td>
                    		<td class='reconciled_deposit_count'><?php echo $count++; ?></td>
                    		<td>
                    			<?php echo timestamp_to_date($unreconciled_withdrawal->transaction_date); ?><br/>
                                <small><strong>Delivered On:</strong> <?php echo timestamp_to_date_and_time($unreconciled_withdrawal->created_on); ?></small><br/>
                                <small><strong>Reconciled On:</strong> <?php echo timestamp_to_date_and_time($unreconciled_withdrawal->modified_on); ?></small>
                    		</td>
                    		<td><?php echo $bank_account_options[$unreconciled_withdrawal->account_number]; ?></td>
                    		<td>
                                <?php echo $unreconciled_withdrawal->particulars; ?> <small><a class='toggle_transaction_alert_details'>More..</a></small>
                                <div class="transaction_alert_details">
                                    <?php echo $unreconciled_withdrawal->description; ?>
                                </div>
                            </td>
                    		<td class="text-right"><?php echo number_to_currency($unreconciled_withdrawal->amount); ?></td>
                    		<!--
        ...
        <p>
            <?php if($unreconciled_withdrawals):?>
...
<?php }else{ ?>
    <div class="alert alert-info">
        ...
        <p>
            No unreconciled withdrawals to display.
        </p>
    </div>
<?php } ?>
<script>
