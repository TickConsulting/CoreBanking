                    ...
                    <div class="m-dropdown__body">              
                        <div class="m-dropdown__content">
                           <?php echo form_open(current_url(),'method="GET" class="m-form m-form--fit m-form--label-align-right"'); ?>
                                ...
                                <div class="form-group m-form__group stocks_search pt-0" >
                                    <label for="">
                                ...
                                <div class="form-group m-form__group stocks_search pt-0" >
                                    ...
                                    <?php echo form_dropdown('stocks[]',array()+$stock_options,$this->input->get('stocks')?:'','class="form-control m-select2" multiple="multiple" '); ?>
                                </div>
        ...
        <?php
          $query = $_SERVER['QUERY_STRING']?'?generate_excel=1&'.$_SERVER['QUERY_STRING']:'?generate_excel=1'; ?>
            <div class="btn-group margin-bottom-20 search-button">
                <a href="<?php echo site_url('group/stocks/stock_sales').$query; ?>" class="btn btn-sm btn-primary m-btn m-btn--icon">
                    <span>
...
<div id="stock_sales_listing" class="table-responsive">
...
<script>
    ...
    $(window).on('load',function(){
        load_stocks_sales_listing();
    });

    function load_stocks_sales_listing(){

        mApp.block('#stock_sales_listing',{
            overlayColor: 'white',
            animate: true
        });
        ...
        $.ajax({
            type: "GET",
            url: '<?php echo base_url("ajax/stocks/get_stock_sales_listing/'+first_uri_segment+'/'+second_uri_segment+'?'+get_string+'"); ?>',
            dataType : "html",
                success: function(response) {
                    $('#stock_sales_listing').html(response);
                    mApp.unblock('#stock_sales_listing');
                }
            }
        );
