                            ...
                            <div class="m-dropdown__content">
                                <?php echo form_open(current_url(),'method="GET" class="m-form m-form--fit m-form--label-align-right"'); ?>
                                    <div class="form-group m-form__group">
                                        <label>
                                            <?php echo translate('Purchase Date Range');?>
                                        </label>
                                    ...
                                    <div class="form-group m-form__group assets_search pt-0" >
                                        <label for="">
                                            <?php echo translate('Select Stocks');?>
                                        </label>
                                        <?php echo form_dropdown('stocks[]',array()+$stock_options,$this->input->get('stocks')?:'','class="form-control select2" multiple="multiple"'); ?>
                                    </div>
        ...
        <?php
          $query = $_SERVER['QUERY_STRING']?'?generate_excel=1&'.$_SERVER['QUERY_STRING']:'?generate_excel=1'; ?>
            <div class="btn-group margin-bottom-20 search-button">
                <a href="<?php echo site_url('group/stocks/listing').$query; ?>" class="btn btn-sm btn-primary m-btn m-btn--icon">
                    <span>
...
<div id="stocks_listing" class="table-responsive" style="min-height: 100px;">
...
function load_stocks_listing(){
    mApp.block('#stocks_listing',{
        overlayColor: 'white',
        animate: true
    });
    $.ajax({
        type: "GET",
        url: '<?php echo base_url("ajax/stocks/get_stocks_listing/'+first_uri_segment+'/'+second_uri_segment+'?'+get_string+'"); ?>',
        dataType : "html",
            success: function(response) {
                $('#stocks_listing').html(response);
                $('.select2').select2({width:"100%"});
                $(document).ready(function() {
                    $('.update_current_price').editable({
                        validate: function(value) {
                            if ($.trim(value) == '') return 'This field is required';
                            regex = /^[0-9,.\b]+$/;
                            if(regex.test($.trim(value))===false){
