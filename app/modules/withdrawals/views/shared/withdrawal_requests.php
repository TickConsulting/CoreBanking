<div class="withdrawal_requests">
    ...
    <div class="tab-content">
        <div class="tab-pane active show" id="approved_requests" role="tabpanel" style="min-height: 180px;"></div>
    ...
    <div class="modal-content">
        <div class="modal-header" style="display: none;">
            <h5 class="modal-title" id="exampleModalLabel">
                <?php echo translate('Funds Transfer'); ?>
...
$(document).ready(function(){
    ...
    $(document).on('click','.get_withdrawal_request',function(){
        ...
        $.ajax({
            type: "GET",
            url: '<?php echo base_url("ajax/withdrawals/get_withdrawal_request/"); ?>'+id+'/'+'<?php echo $this->uri->segment(1, 0);?>',
            dataType : "html",
            success: function(response) {
                if(isJson(response)){
                    var data = $.parseJSON(response);
                    if(data.hasOwnProperty('withdrawal_approval_requests')){
                        ...
                        if(data.withdrawal_request.status == 'is_approved'){
                            ...
                            console.log("created_by : "+data.withdrawal_request.created_by);
                            if(data.withdrawal_request.created_by == '<?php echo $this->user->id?>'){
                                $('.cancel-request').show();
                            ...
                            $('#get_withdrawal_request_modal .badge_holder').html('<span class="m-badge m-badge--warning m-badge--wide"><?php echo translate('Pending Approval') ?></span>');
                        ...
                        if(data.status == "0"){
                            ...
                            $('.withdrawal_requests .error').html('<div class="alert alert-danger alert-dismissible fade show" role="alert"><button type="button" class="close" data-dismiss="alert" aria-label="Close"></button>'+data.message+'</div>');
                        }
                    }else{
                        $('#get_withdrawal_request_modal .close').trigger('click');
