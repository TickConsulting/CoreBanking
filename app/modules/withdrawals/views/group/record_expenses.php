<?php echo form_open($this->uri->uri_string(),'class="m-form m-form--fit m-form--label-align-right m-form--group-seperator-dashed form_submit m-form--state" role="form" id="create_expense_form"'); ?>
    <div class="table-responsive">
        <table class="table table-condensed record_expense_table multiple_payment_entries">
            <thead>
                <tr> 
                    <th width="1%">
                        #
            ...
            <tbody id='append-place-holder'>
                <tr>
                    ...
                    <td>
                        <?php echo form_input('expense_dates[0]',timestamp_to_datepicker(time()),' class="form-control input-sm m-input deposit_date date-picker" readonly="readonly" data-date-format="dd-mm-yyyy" data-date-viewmode="years" data-date-end-date="0d" data-date-start-date="-20y" autocomplete="off" ');?>
                    </td>
                    <td>
                        <span class="m-select2-sm m-input--air">
                            <?php echo form_dropdown('expense_categories[0]',array(''=>translate('Select expense category'))+translate($expense_category_options)+array('0'=>"Add Expense Category"),'',' class="form-control m-input m-select2 expense_category" ');?>
                         </span>
                    </td>
                    <td>
                        <span class="m-select2-sm m-input--air">
                            <?php echo form_dropdown('accounts[0]',array(''=>translate('Select account option'))+translate($account_options)+array('0'=>"Add Account"),'',' class="form-control m-input m-select2  account" ');?>
                        </span>
                    </td>
                    <td>
                        <span class="m-select2-sm m-input--air">
    ...
    <div class="row">
        <div class="col-md-12">
            <a class="btn btn-default btn-sm add-new-line" id="add-new-line">
                <i class="la la-plus"></i><?php echo translate('Add new expense line');?>
            </a>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <span class="float-lg-right float-md-left float-sm-left float-xl-right">
                    <button class="btn btn-primary m-btn m-btn--custom m-btn--icon btn-sm submit_form_button" id="record_expense"  type="submit">
                       <?php echo translate('Record Expense');?>                              
                    </button>
                    &nbsp;&nbsp;
...
<a class="d-none inline" data-toggle="modal" data-target="#create_new_expense_category" data-title="Add Expense Category" data-id="create_expense_category" id="create_expense_category"  >
    <?php echo translate('Add Expense Category');?>
</a>

<div class="modal fade" id="create_new_expense_category" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            ...
            <?php echo form_open($this->uri->uri_string(),'class=" form_submit m-form m-form--state" id="record_expense_category" role="form"'); ?>                              
                <div class="form-group m-form__group ">
                    <div class="form-group m-form__group ">
                        ...
                        <div class="input-group">
                            <?php echo form_input('name',"",'class="form-control" placeholder="Group Expense Category Name" id="name"'); ?>
                        </div>
                    </div>
                    <div class="form-group m-form__group ">
                        <label>
                        ...
                        <div class="input-group">
                           <?php 
                                $textarea = array(
                                    ...
                                    'placeholder'=>'Group Asset Category Description'
                                ); 
                                echo form_textarea($textarea);
                            ?>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <button type="submit" class="btn btn-sm btn-info" id="category_expense_category">
                                <?php echo translate('Save Changes');?>                                         
                            </button>
                           <button type="button" class="btn btn-sm btn-danger" id="cancel_expense_name_modal" data-dismiss="modal">
    ...
    <table>
        <tbody>
            <tr>
                ...
                <td>
                    <?php echo form_input('expense_dates[]',timestamp_to_datepicker(time()),' class="form-control input-sm m-input expense_dates date-picker" readonly="readonly" data-date-format="dd-mm-yyyy" data-date-viewmode="years" data-date-end-date="0d" data-date-start-date="-20y" autocomplete="off" ');?>
                </td>
                <td>
                    <span class="m-select2-sm m-input--air">
                        <?php echo form_dropdown('expense_categories[]',array(''=>'Select expense category')+translate($expense_category_options)+array('0'=>"Add Expense Category"),'',' class="form-control m-input m-select2-append expense_category" ');?>
                     </span>
                </td>
                <td>
                    <span class="m-select2-sm m-input--air">
                        <?php echo form_dropdown('accounts[]',array(''=>'Select account')+translate($account_options),'',' class="form-control m-input m-select2-append account" ');?>
                    </span>
                </td> 
                <td>
                    <span class="m-select2-sm m-input--air">
                        <?php echo form_dropdown('withdrawal_methods[]',array(''=>'Select withdrawal method')+translate($withdrawal_method_options),'',' class="form-control m-input m-select2-append withdrawal_method" ');?>
                    </span>
                </td> 
                <td>
                    <?php 
...
<script type="text/javascript">
    $(document).ready( function(){        
        ...
        SnippetRecordExpense.init();
        $(document).on('submit',"#record_expense_category",function (t) {
            t.preventDefault();
            var e = $(this),
                            ...
                            setTimeout(function () {
                                ...
                                if(validation_errors){
                                    $.each(validation_errors, function( key, value ) {
                                        ...
                                        $('input[name="'+key+'"]').parent().addClass('has-danger');
                                        $('select[name="'+key+'"]').parent().addClass('has-danger');
        ...
        $('.add-new-line').on('click',function(){
            ...
            $('.count').each(function(){
                $(this).text(number);
                $(this).parent().find('.expense_dates').attr('name','expense_dates['+(number-1)+']');
            ...
            $('.record_expense_table .m-select2-append').select2({
                placeholder:{
                    id: '-1',
                    text: "--Select option--",
                }, 
        ...
        $(document).on('click','#create_expense_category',function(){
           $(".member").select2({
                language: 
                    {
                    noResults: function() {
                        return '<a class="inline" data-toggle="modal" data-content="#create_new_expense_category" data-title="Add Expense Category" data-id="create_expense_category" id="create_expense_category" >Add Expense Category</a>';
                    }
                },
        ...
        $(document).on('click','#add_new_account',function(){
            $(".record_expense_table .account").select2({
                language: 
                    {
                     noResults: function() {
    ...
    $(document).ready(function(){
        $('.record_expense_table .expense_category').select2({
            placeholder:{
                id: '-1',
                text: "--Select option--",
            }, 
        ...
        $(".record_expense_table .withdrawal_method").select2({
            placeholder:{
                id: '-1',
                text: "--Select option--",
            }, 
