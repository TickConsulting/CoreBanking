...
class Bank extends Bank_Controller{
    ...
    function listing(){
        ...
        $pagination = create_pagination('bank/withdrawals/listing/pages', $total_rows,50,5,TRUE);
        ...
        $this->template->title(translate('List Withdrawals'))->build('bank/listing',$data);
    }
    ...
    function record_dividend_payments(){
        ...
        if($this->input->post('submit')){
            ...
            if($entries_are_valid){
                ...
                redirect('bank/withdrawals/listing');
            }
            else{   
        ...
        $this->template->title(translate('Record Dividend Payments'))->build('bank/record_dividend_payments',$data);
    ...
    function make_withdrawal(){
        ...
        $this->template->title('Make Withdrawal')->build('bank/make_withdrawal',$this->data);
    ...
    function withdraw_money(){
        ...
        $this->template->title(translate('Withdraw Money'))->build('bank/withdraw_money',$this->data);
    ...
    function record_expenses(){
        ...
        $data['expense_category_options'] = $this->expense_categories_m->get_group_expense_category_options();
        $data['account_options'] = $this->accounts_m->get_active_group_account_options('','','','',FALSE);
        $this->template->title(translate('Record Expenses'))->build('bank/record_expenses',$data);
    }

    function void($id = 0,$redirect = TRUE){
        $id OR redirect('bank/withdrawals/listing');
        $post = $this->withdrawals_m->get_group_withdrawal($id);
        $post OR redirect('bank/withdrawals/listing');
    ...
    function respond($id = 0){
        $id OR redirect("bank/withdrawals/withdrawal_requests");
        $post = $this->withdrawals_m->get_group_member_withdrawal_approval_request_by_member_id($id);
        $post OR redirect("bank/withdrawals/withdrawal_requests");
        if($post->is_approved || $post->is_declined){
            ...
            redirect("bank/withdrawals/withdrawal_requests");
        }
