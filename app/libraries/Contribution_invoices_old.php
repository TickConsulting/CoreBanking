    ...
    */
    ...
    public function queue_regular_contribution_invoices($date = 0){
        ...
        foreach($contributions as $contribution){
            ...
            $invoice_date = $contribution_date - (24*60*60*is_numeric($contribution->invoice_days)?$contribution->invoice_days:0);
            $input = array(
    ...
    public function process_contribution_invoices_queue($limit = 0){
        ...
        $invoices_sent_today_array = $this->ci->invoices_m->get_contribution_invoices_sent_array($invoice_dates_array);
        ...
        foreach($queued_contribution_invoices as $queued_contribution_invoice){
            ...
            if($member&&$contribution&&$contribution_setting){
                if(isset($invoices_sent_today_array[$member->user_id][$member->id][$queued_contribution_invoice->contribution_id][$queued_contribution_invoice->group_id][$queued_contribution_invoice->invoice_date])){ 
        ...
        if($this->ci->transactions->update_group_member_contribution_statement_balances($group_ids,$member_ids)){
    ...
    public function queue_contribution_fine_invoices($date = 0,$ignore_contribution_fine_date = 0,$group_id = 0,$contribution_id = 0,$contribution_fine_setting_id = 0,$contribution_fine_date = 0){
        ...
        if($ignore_contribution_fine_date){
            ...
            $contribution_fine_date = strtotime($fine_date);
        }else{
        ...
        foreach ($contributions as $contribution) {
            ...
            if($member_ids&&$member_contribution_balances_array){
                foreach($member_ids as $member_id){
                    if(isset($member_contribution_balances_array[$member_id])){
                        if($member_contribution_balances_array[$member_id]>0){
                            if($contribution->percentage_fine_mode==1||$contribution->fixed_fine_mode==1){
                                //Find all unpaid invoices
                                if($ignore_contribution_fine_date){
                                    $invoices = $this->ci->invoices_m->get_member_contribution_unpaid_invoices($contribution->group_id,$member_id,$contribution->id,$contribution->fine_limit,$fine_date);
                                }else{
                                    $invoices = $this->ci->invoices_m->get_member_contribution_unpaid_invoices($contribution->group_id,$member_id,$contribution->id,$contribution->fine_limit,$date,$contribution->percentage_fine_chargeable_on,$contribution->fixed_fine_chargeable_on);
                                }
                    ...
                    if($fine_amount_payable>0){
                        ...
                        if($this->ci->transactions->create_contribution_fine_invoice(2,
                            ...
                            $queued_contribution_fine_invoice->parent_invoice_id
                                ...
                                $fine_invoices_sent_today_array[$member->user_id][$member->id][$queued_contribution_fine_invoice->contribution_id][$queued_contribution_fine_invoice->group_id][$queued_contribution_fine_invoice->fine_date][$queued_contribution_fine_invoice->parent_invoice_id] = 1;
