	...
	public function create_bulk($notifications = array()){
		...
		if(empty($notifications)){
			...
			foreach ($notifications as $key => $notification) {
				...
				$from_user = isset($notification->from_user)?$notification->from_user:new stdClass();
				...
				$loan_application_id = isset($notification->loan_application_id)?$notification->loan_application_id:0;
				$withdrawal_id = isset($notification->withdrawal_id)?$notification->withdrawal_id:0;
				// if($subject&&$message&&$from_user&&$from_member_id&&$to_user_id&&$to_member_id&&$group_id&&$call_to_action&&$call_to_action_link&&$category){
				if($subject&&$message&&$to_user_id&&$to_member_id&&$group_id&&$call_to_action&&$call_to_action_link&&$category){
					$notification_entries[] = array(
						'from_member_id' => $from_member_id,
						'from_user_id' => isset($from_user->id)?$from_user->id:'',
						'to_member_id' => $to_member_id,
						'to_user_id' => $to_user_id,
						'group_id' => $group_id,
						'subject' => $subject,
						...
						'is_read' => 0,
						'active' => 1,
						'created_by' => isset($from_user->id)?$from_user->id:'',
						'created_on' => time(),
						'invoice_id' => $invoice_id,
						'deposit_id' => $deposit_id,
						'transaction_alert_id' => $transaction_alert_id,
					...
					$this->ci->session->set_flashdata('error','Could not insert notifications, some parameters are missing');
					break;
					$notification_values_are_valid = FALSE;
				}
			}
	...
	function send_push_notification($user_id = 0,$group_id=0,$message='',$category=0,$invoice_id=0,$deposit_id=0,$transaction_alert_id = 0,$file_size=0,$file_path='',$file_type=1,$reference_number=0,$payment_request_status=0,$withdrawal_request_id=0,$withdrawal_approval_request_id=0,$loan_id = 0,$withdrawal_id=0){
		...
		if($user_id&&$message){
	        ...
	        $payload = array(
				...
				'title'	=> $this->application_settings->application_name,
        	...
        	$fields = array(
				'to'		=> "/topics/".$this->application_settings->application_name,
				'data'	=> $payload
